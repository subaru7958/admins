...
import { fetchPlayerById, updatePlayer } from '../services/playerService';

function PlayerEditModal({ isOpen, player, onClose, onSaved }) {
  ...
  useEffect(() => {
    if (isOpen && player) {
      setIsFetching(true);
      fetchPlayerById(player._id)
        .then(data => {
          setFormData({
            fullName: data.fullName || '',
            dateOfBirth: data.dateOfBirth ? data.dateOfBirth.split('T')[0] : '',
            group: data.group || 'Minimum',
            subgroup: data.subgroup || '',
            email: data.email || '',
            phone: data.phone || '',
            heightCm: (data.heightCm ?? '') === '' ? '' : (data.heightCm ?? ''),
            weightKg: (data.weightKg ?? '') === '' ? '' : (data.weightKg ?? ''),
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-lg">
          <h3 className="text-lg font-medium leading-6 text-gray-900 mb-4">Edit Player</h3>
